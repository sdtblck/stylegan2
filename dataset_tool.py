...
class TFRecordExporter:
    ...
    def create_tfr_writer(self, shape, raw=False):
        if raw:
    ...
    def add_image(self, img, single_lod=False):
        ...
        if self.shape is None:
            ...
            self.res_log2 = int(np.log2(self.shape[1]))
            assert self.shape[0] in [1, 3]
            assert self.shape[1] == self.shape[2]
            assert self.shape[1] == 2 ** self.res_log2
            tfr_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
            if single_lod:
                self.create_tfr_writer(self.shape, raw=False)
            else:
...
def create_from_images_raw(tfrecord_dir, image_dir, shuffle, res_log2=7, resize=None):
    ...
    if resize is not None:
        ...
        try:
            os.mkdir(f"{out_dir}/resized_{resize}/")
        except:
            pass
        ...
        for count, img in enumerate(image_filenames):
            ...
            img.save(f"{out_dir}/resized_{resize}/{fn}")
        image_filenames = sorted(glob.glob(os.path.join(f"{out_dir}/resized_{resize}", '*')))
    if shuffle:
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames), res_log2=res_log2) as tfr:
        ...
        tfr.create_tfr_writer(img.shape, raw=True)
...
def execute_cmdline(argv):
    ...
    p.add_argument('--single_lod', help='Only create a tfr file at a single level of detail (default: 1)', type=int,
                   default=1)
